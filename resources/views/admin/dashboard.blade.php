        ...
        <main class="flex-1 flex flex-col h-full overflow-hidden min-w-0">
            ...
            <header class="bg-[#f8fafc] h-[72px] px-4 md:px-8 flex justify-between items-center shrink-0 border-b border-gray-100">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-lg md:text-2xl font-black text-gray-800 tracking-tight hidden sm:block">Panel de Control Administrador</h2>
                </div>
            ...
            <div class="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 pt-2 custom-scrollbar">

                {{-- ── Tarjetas KPI ──────────────────────────────────────────── --}}
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 mb-6">

                    {{-- Ventas del día --}}
                    <div class="bg-white p-5 md:p-6 rounded-[1.5rem] shadow-sm border border-gray-100">
                        ...
                        <div class="flex items-baseline gap-2 flex-wrap">
                            <span class="text-2xl md:text-3xl font-black text-gray-800">${{ number_format($ventasHoy, 2) }}</span>
                            <span class="text-xs font-bold text-gray-400">{{ $transaccionesHoy }} {{ Str::plural('ticket', $transaccionesHoy) }}</span>
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-5 md:p-6 rounded-[1.5rem] shadow-sm border border-gray-100">
                        ...
                        <span class="text-2xl md:text-3xl font-black text-gray-800">${{ number_format($ingresosMes, 2) }}</span>
                    ...
                    <div class="bg-white p-5 md:p-6 rounded-[1.5rem] shadow-sm border border-red-100">
                        ...
                        <span class="text-2xl md:text-3xl font-black {{ $productosCriticos->count() > 0 ? 'text-red-600' : 'text-gray-400' }}">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8">
                    ...
                    <div class="bg-white rounded-[1.5rem] shadow-sm border border-gray-100 p-5 md:p-6 flex flex-col">
                        <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-3 mb-6">
                            ...
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <div id="fechasPersonalizadas" class="hidden items-center gap-2 flex-wrap">
                    ...
                    <div class="bg-white rounded-[1.5rem] shadow-sm border border-gray-100 p-5 md:p-6">
                        <h3 class="text-lg font-bold text-gray-800 mb-4 md:mb-6 tracking-tight">Productos más Vendidos</h3>
                        <div class="flex flex-col sm:flex-row items-center gap-4 min-h-[200px] md:h-64">
                            <div class="w-full sm:w-1/2 h-48 md:h-full relative shrink-0">
                            ...
                            <div class="w-full sm:w-1/2 space-y-2 md:space-y-3">
                                @php
                                ...
                                @forelse ($productosMasVendidos as $i => $producto)
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-2 min-w-0">
                                            ...
                                            <span class="text-sm font-medium text-gray-600 truncate max-w-[100px] md:max-w-[120px]"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
                    ...
                    {{-- Stock crítico --}}
                    <div class="col-span-1 bg-white rounded-[1.5rem] shadow-sm border border-gray-100 p-5 md:p-6">
                        <h3 class="text-lg font-bold text-gray-800 mb-4 md:mb-6 tracking-tight">Stock Crítico</h3>
                        <div class="space-y-3 md:space-y-4 max-h-64 overflow-y-auto custom-scrollbar">
                            @forelse ($productosCriticos as $producto)
                                <div class="flex items-center gap-3 md:gap-4 p-3 border border-gray-100 rounded-2xl hover:bg-gray-50 transition-colors">
                                    ...
                                    @if ($producto->stock === 0)
                                        <span class="text-[10px] font-bold bg-red-100 text-red-600 px-2 py-0.5 rounded-full shrink-0">AGOTADO</span>
                                    @endif
                                </div>
                            @empty
                    ...
                    {{-- Accesos rápidos --}}
                    <div class="col-span-1 lg:col-span-2 grid grid-cols-2 gap-3 md:gap-4">
                        <a href="{{ route('admin.productos.index') }}" class="bg-white p-4 md:p-6 rounded-2xl border border-gray-100 flex flex-col items-center justify-center gap-2 md:gap-3 hover:border-[#0f763e] transition-all group">
                            <div class="w-10 h-10 md:w-12 md:h-12 bg-green-50 text-[#0f763e] rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                            ...
                            <span class="font-bold text-gray-700 text-sm md:text-base">Inventario</span>
                        </a>
                        <a href="{{ route('admin.ventas.index') }}" class="bg-white p-4 md:p-6 rounded-2xl border border-gray-100 flex flex-col items-center justify-center gap-2 md:gap-3 hover:border-orange-500 transition-all group">
                            <div class="w-10 h-10 md:w-12 md:h-12 bg-orange-50 text-orange-500 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                            ...
                            <span class="font-bold text-gray-700 text-sm md:text-base">Ventas</span>
                        </a>
                        <a href="{{ route('admin.reportes.index') }}" class="bg-white p-4 md:p-6 rounded-2xl border border-gray-100 flex flex-col items-center justify-center gap-2 md:gap-3 hover:border-blue-500 transition-all group">
                            <div class="w-10 h-10 md:w-12 md:h-12 bg-blue-50 text-blue-500 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                                <svg class="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
                            </div>
                            <span class="font-bold text-gray-700 text-sm md:text-base">Reportes</span>
                        </a>
                        <a href="{{ route('admin.usuarios.index') }}" class="bg-white p-4 md:p-6 rounded-2xl border border-gray-100 flex flex-col items-center justify-center gap-2 md:gap-3 hover:border-purple-500 transition-all group">
                            <div class="w-10 h-10 md:w-12 md:h-12 bg-purple-50 text-purple-500 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                            ...
                            <span class="font-bold text-gray-700 text-sm md:text-base">Usuarios</span>
                        </a>
                    </div>
                </div>
            </div>
