...
<body class="bg-[#f8fafc] flex h-screen overflow-hidden font-sans text-gray-800">
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header class="bg-[#f8fafc] h-[72px] px-8 flex justify-between items-center shrink-0 border-b border-gray-100">
            <h2 class="text-2xl font-black text-gray-800 tracking-tight hidden md:block">Admin Portal</h2>
            ...
            <div class="flex-1 max-w-xl mx-4">
                <form action="{{ route('admin.productos.index') }}" method="GET" class="relative" id="form-buscar">
                    ...
                    <input type="text" name="q" value="{{ request('q') }}" id="input-buscar" placeholder="Buscar productos por nombre..." 
        ...
        <div class="flex-1 overflow-y-auto p-8 custom-scrollbar">
            ...
            <div class="flex flex-col md:flex-row md:justify-between md:items-end gap-4 mb-8">
                <div>
                    <h1 class="text-3xl font-black text-gray-800 tracking-tight">Inventario de Productos</h1>
                    <p class="text-gray-500 font-medium mt-1">Gestiona el catálogo, precios y existencias.</p>
                </div>
                <button type="button" id="btn-nuevo-producto" class="bg-[#f97316] hover:bg-[#ea580c] text-white px-5 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 shadow-sm transition-colors shrink-0">
            ...
            <div class="flex flex-wrap items-center gap-2 mb-8">
                <a href="{{ route('admin.productos.index') }}" class="px-5 py-1.5 {{ !request('categoria') && !request('stock') ? 'bg-[#0f763e] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50' }} text-sm font-bold rounded-full shadow-sm transition-colors">Todos</a>
                
                @foreach($categorias as $cat)
                    <a href="{{ route('admin.productos.index', ['categoria' => $cat->id, 'q' => request('q')]) }}" class="px-5 py-1.5 {{ request('categoria') == $cat->id ? 'bg-[#0f763e] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50' }} text-sm font-bold rounded-full transition-colors">{{ $cat->nombre }}</a>
                @endforeach

                <a href="{{ route('admin.productos.index', ['stock' => 'critico', 'q' => request('q')]) }}" class="px-5 py-1.5 {{ request('stock') == 'critico' ? 'bg-red-600 text-white' : 'bg-red-50 text-red-600 hover:bg-red-100' }} text-sm font-bold rounded-full transition-colors flex items-center gap-1.5 ml-2">
                ...
                @forelse($productos as $producto)
                    ...
                    <div class="bg-white rounded-[1.5rem] border {{ $isCriticalStock ? 'border-red-100' : 'border-gray-100' }} shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow relative {{ $isOutStock ? 'opacity-75 grayscale hover:grayscale-0' : '' }}">
                        ...
                        <div class="p-5 flex flex-col flex-1">
                            ...
                            <h3 class="text-base font-bold text-gray-800 leading-tight mb-4 line-clamp-2">{{ $producto->nombre }}</h3>
                            
                            <div class="flex items-center justify-between mb-5 mt-auto">
                                <span class="text-2xl font-black {{ $isOutStock ? 'text-gray-500' : 'text-[#0f763e]' }}">${{ number_format($producto->precioActual->precio ?? 0, 2) }}</span>
