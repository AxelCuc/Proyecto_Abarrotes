...
<body 
    class="bg-[#f8fafc] font-sans text-gray-800"
    x-data="{ ...usuariosHandler(), sidebarOpen: false }"
>
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <main class="flex-1 flex flex-col h-full overflow-hidden min-w-0">
            ...
            <div class="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 pt-4 md:pt-6 custom-scrollbar">
                ...
                <div class="flex flex-col sm:flex-row justify-between sm:items-end gap-4 mb-6 md:mb-8">
                    <div>
                        <h1 class="text-2xl md:text-3xl font-black text-gray-800 tracking-tight">Gestión de Usuarios & Roles</h1>
                        <p class="text-gray-500 font-medium mt-1 text-sm md:text-base">Administra los accesos y permisos del personal de la tienda.</p>
                    </div>
                    <button @click="openCreateModal = true" class="w-full sm:w-auto justify-center bg-[#f97316] hover:bg-[#ea580c] text-white px-5 md:px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-md transition-all shrink-0">
                ...
                <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden flex flex-col">
                    ...
                    <div class="p-4 md:p-5 border-b border-gray-100 flex items-center justify-between">
                        <form method="GET" action="{{ route('admin.usuarios.index') }}" class="relative w-full sm:max-w-sm">
                        ...
                        <table class="w-full text-left border-collapse min-w-[700px]">
                            <thead>
                                <tr class="bg-gray-50 border-b border-gray-100">
                                    <th class="px-4 md:px-6 py-3 md:py-4 text-[10px] md:text-[11px] font-bold text-gray-500 uppercase tracking-wider">Nombre Completo</th>
                                    <th class="px-4 md:px-6 py-3 md:py-4 text-[10px] md:text-[11px] font-bold text-gray-500 uppercase tracking-wider">Email</th>
                                    <th class="px-4 md:px-6 py-3 md:py-4 text-[10px] md:text-[11px] font-bold text-gray-500 uppercase tracking-wider">Rol Asignado</th>
                                    <th class="px-4 md:px-6 py-3 md:py-4 text-[10px] md:text-[11px] font-bold text-gray-500 uppercase tracking-wider">Estado</th>
                                    <th class="px-4 md:px-6 py-3 md:py-4 text-[10px] md:text-[11px] font-bold text-gray-500 uppercase tracking-wider text-right">Acciones</th>
                                </tr>
                            </thead>
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors group">
                                    <td class="px-4 md:px-6 py-3 md:py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            ...
                                            @endphp
                                            <div class="w-8 h-8 md:w-10 md:h-10 rounded-full {{ $colorClass }} flex items-center justify-center font-bold text-xs md:text-sm shrink-0">
                                                {{ $initials }}
                                            </div>
                                            <div>
                                                <p class="text-sm font-bold text-gray-900">{{ $usuario->nombre }}</p>
                                                <p class="text-[10px] md:text-xs text-gray-400 mt-0.5">ID: {{ str_pad($usuario->id, 6, '0', STR_PAD_LEFT) }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 md:px-6 py-3 md:py-4 whitespace-nowrap">
                                        <p class="text-xs md:text-sm text-gray-600 font-medium">{{ $usuario->email }}</p>
                                    </td>
                                    <td class="px-4 md:px-6 py-3 md:py-4 whitespace-nowrap">
                                        @php
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-2.5 md:px-3 py-0.5 md:py-1 rounded-full text-[10px] md:text-[11px] font-bold uppercase tracking-wider {{ $badgeClass }}">
                                            {{ $rolNombre }}
                                        </span>
                                    </td>
                                    <td class="px-4 md:px-6 py-3 md:py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <span class="text-xs md:text-sm font-medium text-gray-700">{{ $usuario->activo ? 'Activo' : 'Inactivo' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-4 md:px-6 py-3 md:py-4 whitespace-nowrap text-right">
                                        <div class="flex justify-end gap-1 md:gap-2">
                                            <button @click="editUser({{ $usuario->id }}, '{{ addslashes($usuario->nombre) }}', '{{ $usuario->email }}', '{{ $rolNombre }}')" class="p-1.5 md:p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Editar Usuario">
                                            ...
                                            <button @click="deleteUser({{ $usuario->id }}, '{{ addslashes($usuario->nombre) }}')" class="p-1.5 md:p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Eliminar Usuario">
